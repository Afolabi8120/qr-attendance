<?php
	...
	if(isset($_POST['btnRegister']) AND !empty($_POST['btnRegister'])){
		...
		if(empty($matricno) || empty($fullname) || empty($email) || empty($phone) || empty($password) || empty($cpassword)){
			$_SESSION['ErrorMessage'] = "All Fields are Required";
		}else if(!preg_match("/^[a-zA-Z0-9-]*$/", $matricno)){
			$_SESSION['ErrorMessage'] = "Use a Valid Staff No";
			...
			$hash_password = sha1(md5($password));
 			...
 			if(!in_array($file_extension, $validextensions)){ 
            	...
            	if($user->register($matricno,$fullname,$email,$phone,'','','','','',$pass,$image_name,'Lecturer','Active') === true){
